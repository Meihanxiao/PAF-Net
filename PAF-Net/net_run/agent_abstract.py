...
from  net_run.get_optimizer import get_optimiser
...
class NetRunAgent(object):
    ...
    def create_dataset(self):
        if(self.stage == 'train'):
            ...
            bn_valid = self.config['dataset'].get('valid_batch_size', 16)
            num_worker = self.config['dataset'].get('num_workder', 1)
            self.train_loader = torch.utils.data.DataLoader(self.train_set, 
                batch_size = bn_train, shuffle=True, num_workers= num_worker,
                # batch_size = bn_train, shuffle=True, num_workers= 8,
            ...
            self.test_loder = torch.utils.data.DataLoader(self.test_set, 
                batch_size = bn_test, shuffle=False, num_workers=1)
    ...
    def create_optimizer(self, params):
        ...
        if(self.scheduler is None):
            # self.scheduler = optim.lr_scheduler.CosineAnnealingLR(self.optimizer, 
                # self.config['training']['iter_max']*self.config['dataset']['train_batch_size'], eta_min=1e-6)
