...
def create_csv_file(data_root, output_file, fields):
    ...
    filenames = []
    tmp='/picture/data'
    patient_names = os.listdir(data_root +tmp+ '/' + fields[0])
    ...
    for patient_name in patient_names:
        ...
        for field in fields:
            ...
            patient_name=str(patient_name[:-4])+end
            image_name = data_root +tmp+ '/' + field + '/' + patient_name
...
def random_split_dataset():
    random.seed(2019)
    input_file = 'dataset/config/jsrt_all.csv'
    train_names_file = 'dataset/config/jsrt_train.csv'
    valid_names_file = 'dataset/config/jsrt_valid.csv'
    test_names_file  = 'dataset/config/jsrt_test.csv'
    with open(input_file, 'r') as f:
        lines = f.readlines()
    data_lines = lines[1:]
    shuffle(data_lines)
    train_lines  = data_lines[:0]
    valid_lines  = data_lines[0:6838]
    test_lines   = data_lines[0:6838]
...
if __name__ == "__main__":
    ...
    JSRT_root   = 'dataset'
    output_file = 'dataset/config/jsrt_all.csv'
    fields      = ['image', 'label']
    ...
    gt_seg_csv  = "dataset/config/jsrt_test_gt_seg.csv"
    get_evaluation_image_pairs(test_csv, gt_seg_csv)
    # for i in range(200):
    #     img=cv2.imread('dataset/picture/data/image(200+98)/'+str(i)+'.jpg')
    #     gth=cv2.imread('dataset/picture/data/label(200+98)/'+str(i)+'.png')
    #     cv2.imwrite('dataset/picture/data/image/'+str(1600+i)+'.jpg',img)
    #     cv2.imwrite('dataset/picture/data/label/'+str(1600+i)+'.png',gth)
    #     cv2.waitKey(0)
